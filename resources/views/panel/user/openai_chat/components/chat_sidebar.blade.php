<div class="chats-list-container flex flex-col overflow-hidden w-1/4 shrink-0 grow-0 border-l-0 border-b-0 border-t-0 border-solid border-[--tblr-border-color] max-md:w-full max-md:h-[50vh]">
    ...
    <div class="px-6 mt-auto chats-new py-11 max-xl:p-4">
        <a href="javascript:void(0);" @if(env('APP_STATUS') == "Demo" && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'))) onclick="return toastr.info('This feature is disabled in Demo version.')" @else onclick="return startNewChat({{$category->id}}, '{{LaravelLocalization::getCurrentLocale()}}');" @endif class="btn btn-primary flex items-center justify-center w-full py-2 gap-[10px]">
