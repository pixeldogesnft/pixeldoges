    ...
    @foreach ($list as $entry)
		...
		id="chat_{{$entry->id}}">
			...
			<span class="absolute -translate-y-1/2 top-1/2 end-4">
				<button @if(env('APP_STATUS') == "Demo" && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'))) onclick="return toastr.info('This feature is disabled in Demo version.')"  @endif class="@if(env('APP_STATUS') == "Demo" && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'))) @else  chat-item-update-title @endif btn bg-[--tblr-body-bg] w-[28px] h-[28px] p-0 border-[1px] border-solid border-[--tblr-border-color] opacity-0 group-hover:!opacity-100 group-[&.edit-mode]:!opacity-100 group-[&.edit-mode]:!bg-[--tblr-green]  dark:bg-[rgba(255,255,255,0.08)]">
				...
				<button  @if(env('APP_STATUS') == "Demo" && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'))) onclick="return toastr.info('This feature is disabled in Demo version.')"  @endif class="@if(env('APP_STATUS') == "Demo" && (isset($category) && ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf'))) @else chat-item-delete @endif btn bg-red w-[28px] h-[28px] p-0 border-[1px] border-solid border-red opacity-0 group-hover:!opacity-100 group-[&.edit-mode]:hidden">
